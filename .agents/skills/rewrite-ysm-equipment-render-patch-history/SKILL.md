---
name: rewrite-ysm-equipment-render-patch-history
description: Safely prepare, back up, and verify an explicitly requested YSM-Equipment_Render-Patch Git history rewrite such as amend, squash, rebase, commit reconstruction, or retroactive correction. Use only when the user explicitly requests rewriting existing history. Do not use for ordinary commits, merges, branch creation, or pushes.
---

# Rewrite YSM-Equipment_Render-Patch History
...
```powershell
& .\.agents\skills\rewrite-ysm-equipment-render-patch-history\scripts\history-safety.ps1 -Operation Inspect -Branch <affected branches>
```
